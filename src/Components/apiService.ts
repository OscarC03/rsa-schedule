...
class ApiService {
  ...
  /**
   * Salva matrice turni nel database
   */
  async saveMatrix(year: number, month: number, matrix: any): Promise<ApiResponse> {
    return this.request('/matrix.php', {
      method: 'POST',
      body: JSON.stringify({
        year,
        month,
  ...
  /**
   * Carica matrice turni dal database
   */
  async getMatrix(year: number, month: number): Promise<ApiResponse<any>> {
    return this.request<any>(`/matrix.php?year=${year}&month=${month}`);
  }

  /**
   * Elimina matrice turni dal database
   */
  async deleteMatrix(year: number, month: number): Promise<ApiResponse> {
    return this.request(`/matrix.php?year=${year}&month=${month}`, {
      method: 'DELETE'
    });
  }
}
